...
#### Elementwise Kernels
- [x] Create `backend/ops_elementwise.cu`
  - [x] Implement `add_kernel`: `__global__ void add_kernel(float* a, float* b, float* c, int n)`
  - [x] Implement `mul_kernel`, `relu_kernel`
  - [x] Implement `add_backward_kernel`, `mul_backward_kernel`, `relu_backward_kernel`
  - [x] Each kernel: 1 thread per element, grid-stride loop
...
#### Testing
- [x] Update `tests/test_gpu_ops.py`
  - [x] Test CPU vs CUDA correctness for `add`, `mul`, `relu`
